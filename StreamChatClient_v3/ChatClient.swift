...
public class Client<ExtraData: ExtraDataTypes> {
    ...
    public func setAnonymousUser(shouldConnectAutomatically: Bool = true, completion: ((Error?) -> Void)? = nil) {
        disconnect()
    ...
    private func prepareEnvironmentForNewUser(userId: UserId,
        ...
        webSocketClient.endpoint = webSocketConnectEndpoint(userId: userId, role: role, extraData: extraData)
    ...
    private func webSocketConnectEndpoint(userId: UserId, role: UserRole = .user,
        ...
        // Create a connection request
        let socketPayload = WebSocketPayload<ExtraData.User>(userId: currentUserId, userRole: role, extraData: extraData)
