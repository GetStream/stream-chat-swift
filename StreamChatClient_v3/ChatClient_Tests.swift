...
class ChatClient_Tests: XCTestCase {
    ...
    func test_settingAnonymousUser() {
        ...
        let oldWSConnectEndpoint = client.webSocketClient.endpoint
        ...
        AssertAsync {
            ...
            Assert.willBeTrue(AnyEndpoint(oldWSConnectEndpoint) != AnyEndpoint(client.webSocketClient.endpoint))
...
class DatabaseContainerMock: DatabaseContainer {
    ...
    override func flush(force: Bool, completion: ((Error?) -> Void)? = nil) {
        flush_called = true
        super.flush(force: force, completion: completion)
    }
}
