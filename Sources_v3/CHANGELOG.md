...
**`StreamChat` SDK reaches another milestone with the version 3.0!**
...
Currently, the beta version is available only using **CocoaPods**. We will support all common dependency managers by the time the version 3 reaches the public release.

To use the beta version, add to your `Podfile`:
```ruby
...
The framework naming and overall structure was changed. Since version 3.0, Stream Chat iOS SDK consists of:

**`StreamChat` framework**

Contains the low-level logic and is meant to be used by users who want to build fully custom UI . It covers functionality previously provided by `StreamChatCore` and `StreamChatClient`.

**`StreamChat UI` framework** (release planned 10/2020)

Contains a complete set of ready-to-use configurable UI elements which you can customize a use for building your own chat UI. It covers functionality previously provided by `StreamChat`.

### Sample App

The best way to explore the SDKs and their usage is our sample app. It contains example implementation of a simple IRC-style chat app using the following patterns:
...
Lear more about the sample app at its own [README](https://github.com/GetStream/stream-chat-swift/tree/master/Sample_v3).

### Cheat Sheet

[Cheat sheet](https://github.com/GetStream/stream-chat-swift/wiki/StreamChat-SDK-Cheat-Sheet) is something like FAQ but with code snippets and common solutions. It's one of the best ways to get the general idea of how to use the SDK, how common problems are solved, and what are the best patterns for the SDK usage.

We keep the cheat sheet up to date and expand it regularly based on your questions and comments.
