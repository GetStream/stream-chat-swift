...
//        if message.repliesTo != nil {
    ...
    private func didSelectMessageCell(_ cell: СhatMessageCollectionViewCell<ExtraData>) {
        ...
        actionsController.delegate = .init(
            didTapOnInlineReply: { [weak self] in
                guard let self = self else { return }
                self.delegate?.didTapOnInlineReply?(self, $1)
                self.dismiss(animated: true)
            },
            didTapOnThreadReply: { [weak self] _, _ in
                self?.dismiss(animated: true)
            },
            didTapOnEdit: { [weak self] in
                guard let self = self else { return }
                self.delegate?.didTapOnEdit?(self, $1)
                self.dismiss(animated: true)
            },
