...
final class ChatMessageVoiceRecordingAttachmentListViewItemView_Tests: XCTestCase {
    ...
    func test_setUpLayout_playbackLoadingClampedViewWasConfiguredCorrectly() {
        XCTAssertTrue(subject.playbackLoadingClampedView.superview === subject.informationStack)
    ...
    func test_setUpLayout_informationStackWasConfiguredCorrectly() {
        XCTAssertTrue(subject.informationStack.superview === subject.fileNameAndInformationStack)
        XCTAssertEqual(subject.informationStack.axis, .horizontal)
        XCTAssertEqual(subject.informationStack.spacing, 4)
        XCTAssertEqual(subject.informationStack.alignment, .center)
        XCTAssertEqual(subject.informationStack.subviews, [
    ...
    func test_setUpLayout_fileNameAndInformationStackWasConfiguredCorrectly() {
        XCTAssertTrue(subject.fileNameAndInformationStack.superview === subject.mainContainerStackView)
        XCTAssertEqual(subject.fileNameAndInformationStack.axis, .vertical)
        XCTAssertEqual(subject.fileNameAndInformationStack.spacing, 8)
        XCTAssertEqual(subject.fileNameAndInformationStack.alignment, .fill)
        XCTAssertEqual(subject.fileNameAndInformationStack.arrangedSubviews, [
            subject.fileNameLabel,
            subject.informationStack
        ])
    }
    ...
    func test_setUpLayout_mainContainerStackViewWasConfiguredCorrectly() {
        ...
        XCTAssertEqual(subject.mainContainerStackView.arrangedSubviews, [
            ...
            subject.fileNameAndInformationStack,
    ...
    func test_updateContent_contentIsNotNil_uploadingStateIsUploaded_durationLabelWasConfiguredCorrectly() {
        subject.content = .mock(id: .unique, assetURL: .unique(), localState: .uploaded, extraData: ["duration": .number(90)])
        subject.updateContent()

        XCTAssertEqual(subject.durationLabel.text, "01:30")
    }

    func test_updateContent_contentIsNotNil_uploadingStateIsNil_durationLabelWasConfiguredCorrectly() {
        subject.content = .mock(id: .unique, assetURL: .unique(), localState: nil, extraData: ["duration": .number(90)])
        subject.updateContent()
    ...
    func test_updateContent_contentIsNotNil_uploadingStateIsUploaded_waveformWasConfiguredCorrectly() {
        subject.content = .mock(id: .unique, assetURL: .unique(), localState: .uploaded, extraData: [
            "duration": .number(90),
            "waveform": .array([.number(1), .number(2)])
        ])
        subject.updateContent()
