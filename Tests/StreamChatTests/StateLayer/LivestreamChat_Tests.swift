...
final class LivestreamChat_Tests: XCTestCase {
    ...
    func test_get_whenAPIFails_thenErrorIsThrown() async {
        ...
        client.mockAPIClient.test_mockResponseResult(Result<ChannelStateResponse, Error>.failure(testError))
    ...
    func test_get_callsCorrectAPIEndpoint() async throws {
        ...
        let expectedEndpoint = Endpoint<ChannelStateResponse>.channelQuery(channelQuery)
    ...
    func test_stopWatching_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(Result<Response, Error>.success(Response(duration: "")))

        try await livestreamChat.stopWatching()

        let expectedEndpoint = Endpoint<Response>.stopWatchingChannel(type: channelQuery.cid!.type.rawValue, id: channelQuery.cid!.id)
        XCTAssertEqual(client.mockAPIClient.request_endpoint, AnyEndpoint(expectedEndpoint))
    }
    ...
    func test_deleteMessage_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(
            Result<DeleteMessageResponse, Error>.success(DeleteMessageResponse(duration: "", message: .dummy(messageId: "msg-1")))
        )

        try await livestreamChat.deleteMessage("msg-1")

        let expectedEndpoint = Endpoint<DeleteMessageResponse>.deleteMessage(id: "msg-1", hard: false, deletedBy: nil, deleteForMe: nil)
        XCTAssertEqual(client.mockAPIClient.request_endpoint, AnyEndpoint(expectedEndpoint))
    }
    ...
    func test_deleteMessage_withHardTrue_callsCorrectAPI() async throws {
        ...
        let expectedEndpoint = Endpoint<DeleteMessageResponse>.deleteMessage(id: "msg-1", hard: true, deletedBy: nil, deleteForMe: nil)
        XCTAssertEqual(client.mockAPIClient.request_endpoint, AnyEndpoint(expectedEndpoint))
    }
    ...
    func test_flagMessage_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(
            Result<FlagResponse, Error>.success(FlagResponse(duration: "", itemId: "msg-1"))
        )

        try await livestreamChat.flagMessage("msg-1", reason: "spam", extraData: ["k": .string("v")])

        let expectedEndpoint = Endpoint<FlagResponse>.flagMessage(
            true,
            with: "msg-1",
            reason: "spam",
    ...
    func test_sendReaction_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(
            Result<SendReactionResponse, Error>.success(
                SendReactionResponse(
                    duration: "",
                    message: .dummy(messageId: "msg-1"),
                    reaction: .dummy(messageId: "msg-1", user: .dummy(userId: .unique))
                )
            )
        )
        ...
        let expectedEndpoint = Endpoint<SendReactionResponse>.sendReaction(
            ...
            sendReactionRequest: SendReactionRequest(
                enforceUnique: true,
                reaction: ReactionRequest(custom: ["k": .string("v")], score: 2, type: reactionType.rawValue),
                skipPush: true
            )
        )
        XCTAssertEqual(client.mockAPIClient.request_endpoint, AnyEndpoint(expectedEndpoint))
    ...
    func test_deleteReaction_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(
            Result<DeleteReactionResponse, Error>.success(
        ...
        let expectedEndpoint = Endpoint<DeleteReactionResponse>.deleteReaction(id: "msg-1", type: reactionType.rawValue, userId: nil)
        XCTAssertEqual(client.mockAPIClient.request_endpoint, AnyEndpoint(expectedEndpoint))
    }
    ...
    func test_loadReactions_callsCorrectAPIAndReturnsReactions() async throws {
        ...
        let payload = QueryReactionsResponse(duration: "", reactions: [reactionPayload])
        client.mockAPIClient.test_mockResponseResult(Result<QueryReactionsResponse, Error>.success(payload))

        let reactions = try await livestreamChat.loadReactions(for: messageId, limit: 10, offset: 5)

        let expectedEndpoint = Endpoint<QueryReactionsResponse>.queryReactions(
            id: messageId,
            queryReactionsRequest: QueryReactionsRequest(limit: 10)
        )
        XCTAssertEqual(client.mockAPIClient.request_endpoint, AnyEndpoint(expectedEndpoint))
    ...
    func test_pinMessage_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(Result<UpdateMessagePartialResponse, Error>.success(UpdateMessagePartialResponse(duration: "")))
    ...
    func test_loadPinnedMessages_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(
            Result<MessageListPayload, Error>.success(.init(messages: []))
        )
        ...
        let expectedEndpoint = Endpoint<MessageListPayload>.pinnedMessages(
    ...
    func test_enableSlowMode_callsCorrectAPI() async throws {
        client.mockAPIClient.test_mockResponseResult(Result<UpdateChannelPartialResponse, Error>.success(.init(duration: "", members: [])))
...
private extension LivestreamChat_Tests {
    ...
    func makeChannelQueryWithoutCid() -> ChannelQuery {
        ChannelQuery(
            id: nil,
            type: .messaging,
            channelPayload: ChannelInput(custom: [:], members: [])
        )
    }
