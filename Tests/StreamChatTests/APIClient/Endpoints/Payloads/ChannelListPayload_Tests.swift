...
final class ChannelListPayload_Tests: XCTestCase {
    ...
    func test_decode_bigChannelListPayload_V3() {
        // 3MB JSON Channel List from Watercooler
        let url = XCTestCase.mockData(fromJSONFile: "new_format_v3")

        measure {
            do {
                let start = Date()
                let data = try JSONDecoder.default.decode(QueryChannels_V3.self, from: url)
                let end = Date()
                let diff = end.timeIntervalSince(start)
                print("time: \(diff)")
            } catch {
...
struct Message_V3: Decodable {
    ...
    let latest_reactions: [MessageReactionPayload]
    let own_reactions: [MessageReactionPayload]
//    let reaction_counts: [MessageReactionType: Int]
//    let reaction_scores: [MessageReactionType: Int]
    let reply_count: Int
