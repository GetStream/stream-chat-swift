...
final class ChatClient_Tests: XCTestCase {
    ...
    func test_webSocketClientConfiguration() throws {
        ...
        XCTAssert(client.webSocketEncoder?.connectionDetailsProviderDelegate === client)
    }
    ...
    func test_queryGroupedChannels_withEmptyGroups_sendsFetchAllBody() async throws {
        ...
        client.mockAPIClient.test_mockResponseResult(.success(GroupedQueryChannelsResponse(duration: "", groups: [:])))
    ...
    func test_logout_whenCurrentDevice_removesDevice() throws {
        ...
        waitForExpectations(timeout: defaultTimeout)
        XCTAssertCall(ConnectionRepository_Mock.Signature.disconnect, on: testEnv.connectionRepository!)
        XCTAssertEqual(testEnv.apiClient?.request_endpoint?.path.value, EndpointPath.deleteDevice.value)
        ...
        for (queue, expectation) in queueAndExpectation {
            ...
            queue.async {
                ...
                let expectedWebSocketEndpoint = AnyEndpoint(
                    Endpoint<EmptyResponse>.webSocketConnect()
                )
                // 1. Check `currentUserId` is fetched synchronously
                // 2. `webSocket` has correct connect endpoint and user info
                if chatClient.currentUserId == currentUserId,
                   chatClient.connectionRepository.webSocketConnectEndpoint.value.map(AnyEndpoint.init) == expectedWebSocketEndpoint,
                   chatClient.connectionRepository.connectUserInfo.value?.id == currentUserId {
                    expectation.fulfill()
                }
            }
        }
...
private class TestEnvironment {
    ...
    @Atomic var notificationCenter: PersistentEventNotificationCenter?
    ...
    lazy var environment: ChatClient.Environment = { [unowned self] in
        .init(
            ...
            connectionRepositoryBuilder: {
                self.connectionRepository = ConnectionRepository_Mock(
                    isClientInActiveMode: $0,
                    syncRepository: $1,
                    webSocketEncoder: $2,
                    webSocketClient: $3,
