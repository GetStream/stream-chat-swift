...
final class EventNotificationCenter_Tests: XCTestCase {
    ...
    func test_measure_processMultipleNewMessageEvents() throws {
        let existingPayloads: [MessageResponse] = (0...200).map { _ in
            MessageResponse.dummy(messageId: .unique, authorUserId: .unique)
        }
    ...
    func test_registerManualEventHandling_callsManualEventHandler() {
        ...
        let center = PersistentEventNotificationCenter(database: database, manualEventHandler: mockHandler)
        let cid: ChannelId = .unique
