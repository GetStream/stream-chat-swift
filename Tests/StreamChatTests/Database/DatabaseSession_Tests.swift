...
final class DatabaseSession_Tests: XCTestCase {
    ...
    func test_saveEvent_whenMessageUpdated_shouldSaveMessageWithRestrictedVisibilityLocallyButNotAddingItToTheChannel() throws {
        ...
        try database.readSynchronously { session in
            ...
            // Message is not associated with the channel
            XCTAssertFalse(channelDTO.messages.contains(where: { $0.id == messageId }))
            XCTAssertFalse(channelDTO.pinnedMessages.contains(where: { $0.id == messageId }))
            // But the message is locally available
