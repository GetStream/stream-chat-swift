...
final class PollsRepository_Tests: XCTestCase {
    ...
    func test_createPoll_whenSuccessfull() {
        ...
        let payload = XCTestCase().dummyPollResponseData()
        let response = PollResponse(poll: payload)
    ...
    func test_suggestPollOption_whenSuccess() {
        ...
        let pollId = "123"
        let payload = XCTestCase().dummyPollOptionResponseData(text: pollOption)
    ...
    func test_castPollVote_whenSuccess() throws {
        ...
        let payload = XCTestCase().dummyPollVoteResponseData(optionId: pollOptionId, pollId: pollId)
        let response = PollVoteResponse.dummy(vote: payload)
        apiClient.test_simulateResponse(.success(response))
    ...
    func test_castPollAnswer_whenSuccess() {
        ...
        let payload = XCTestCase().dummyPollVoteResponseData(answerText: answer, pollId: pollId)
        let response = PollVoteResponse.dummy(vote: payload)
        apiClient.test_simulateResponse(.success(response))
    ...
    func test_removePollVote_whenSuccess() throws {
        ...
        let response = PollVoteResponse.dummy()
        apiClient.test_simulateResponse(.success(response))
    ...
    func test_queryPollVotes_whenSuccess() {
        ...
        let vote = XCTestCase().dummyPollVoteResponseData()
        let response = PollVotesResponse.dummy(votes: [vote])
        apiClient.test_simulateResponse(.success(response))
