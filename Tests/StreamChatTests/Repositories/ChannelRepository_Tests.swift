...
final class ChannelRepository_Tests: XCTestCase {
    ...
    func test_getChannel_storeFalse_errorResponse() throws {
        ...
        apiClient.test_mockResponseResult(Result<ChannelStateResponse, Error>.failure(expectedError))
        let result = try waitFor { done in
            repository.getChannel(for: query, store: false, completion: done)
        }
        let expectedEndpoint = Endpoint<ChannelStateResponse>.channelQuery(query, requiresConnectionId: true)
    ...
    func test_markReadLocally_updatesLastReadAtInDatabase() throws {
        let cid = ChannelId.unique
        let currentUserResponse = UserResponse.dummy(userId: .unique)

        try database.writeSynchronously { session in
            try session.saveCurrentUser(payload: .dummy(userId: currentUserResponse.id, role: .user))
            try session.saveChannel(payload: .dummy(channel: .dummy(cid: cid)))
            _ = session.loadOrCreateChannelRead(cid: cid, userId: currentUserResponse.id)
        }

        let beforeMark = Date()
        let expectation = self.expectation(description: "markReadLocally completes")
        repository.markReadLocally(cid: cid, userId: currentUserResponse.id) { _ in
            expectation.fulfill()
        }
        waitForExpectations(timeout: defaultTimeout)
