...
final class WebSocketConnectPayload_Tests: XCTestCase {
    let token = Token.unique(userId: "tommaso")
    ...
    func test_encodesWebSocket_whenUserInfoWithDefaultValues() throws {
        let payload = WSAuthMessage(token: token, userInfo: .init(id: "tommaso"))
        let serialized = try JSONEncoder.stream.encode(payload)
    ...
    func test_EncodesWebSocket_whenConnectPayloadHasNoImage() throws {
        ...
        let payload = WSAuthMessage(token: token, userInfo: .init(id: "tommaso", name: "tommaso", imageURL: nil, extraData: custom))
    ...
    func test_encodesWebSocket_whenConnectPayloadHasNoInvisible() throws {
        ...
        let payload = WSAuthMessage(token: token, userInfo: .init(
            id: "tommaso",
            name: "tommaso",
            imageURL: nil,
            isInvisible: nil,
