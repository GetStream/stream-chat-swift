...
final class ChannelEvents_Tests: XCTestCase {
    ...
    func test_updated_usingServerSideAuth() throws {
        let json = XCTestCase.mockData(fromFile: "ChannelUpdated_ServerSide", bundle: .testToolsBundle)
        let event = try eventDecoder.decode(from: json) as? ChannelUpdatedEventDTO
        XCTAssertEqual(event?.channel.cid, ChannelId(type: .messaging, id: "new_channel_7070"))
    ...
    func test_ChannelHiddenEvent_decoding() throws {
        var json = XCTestCase.mockData(fromFile: "ChannelHidden", bundle: .testToolsBundle)
        ...
        json = XCTestCase.mockData(fromFile: "ChannelHidden+HistoryCleared", bundle: .testToolsBundle)
    ...
    func test_channelTruncatedEvent() throws {
        let mockData = XCTestCase.mockData(fromFile: "ChannelTruncated", bundle: .testToolsBundle)
    ...
    func test_channelTruncatedEventWithMessage() throws {
        let mockData = XCTestCase.mockData(fromFile: "ChannelTruncated_with_message", bundle: .testToolsBundle)
