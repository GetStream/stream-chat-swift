...
final class AIIndicatorEvents_Tests: XCTestCase {
    ...
    func test_aiIndicatorUpdate() throws {
        ...
        let dto = try XCTUnwrap(
            try eventDecoder.decode(from: json).unwrappedEvent as? AIIndicatorUpdateEventDTO
        )
    ...
    func test_aiIndicatorClear() throws {
        ...
        let dto = try XCTUnwrap(
            try eventDecoder.decode(from: json).unwrappedEvent as? AIIndicatorClearEventDTO
        )
        XCTAssertEqual(
            dto.cid,
    ...
    func test_aiIndicatorStop() throws {
        ...
        let dto = try XCTUnwrap(
            try eventDecoder.decode(from: json).unwrappedEvent as? AIIndicatorStopEventDTO
        )
        XCTAssertEqual(
            dto.cid,
