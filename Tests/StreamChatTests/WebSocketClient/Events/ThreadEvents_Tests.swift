...
final class ThreadEvents_Tests: XCTestCase {
    ...
    func test_threadUpdated() throws {
        ...
        let event = try XCTUnwrap(try eventDecoder.decode(from: json).unwrappedEvent as? ThreadUpdatedEventDTO)
    ...
    func test_threadMessageNew() throws {
        ...
        let event = try XCTUnwrap(try eventDecoder.decode(from: json).unwrappedEvent as? NotificationThreadMessageNewEventDTO)
        ...
        XCTAssertEqual(event.unreadThreads, 7)
    }
}
