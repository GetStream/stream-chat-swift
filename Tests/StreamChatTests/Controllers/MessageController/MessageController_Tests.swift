...
final class MessageController_Tests: XCTestCase {
    ...
    func test_createReminder_propagatesSuccessResult() {
        ...
        // Setup mock response from message updater
        env.messageUpdater?.createReminder_completion_result = .success(reminderResponse)

        // Setup callback verification
        let exp = expectation(description: "createReminder callback should be called")
        ...
        // Assert messageUpdater is called with correct params
        XCTAssertNotNil(env.messageUpdater)
        XCTAssertEqual(env.messageUpdater?.createReminder_messageId, messageId)
        XCTAssertEqual(env.messageUpdater?.createReminder_cid, cid)
        XCTAssertEqual(env.messageUpdater?.createReminder_remindAt, remindAt)
        XCTAssertEqual(receivedResult?.value?.id, reminderResponse.id)
    }

    func test_createReminder_propagatesFailureResult() {
        // Setup mock error response from message updater
        let testError = TestError()
        env.messageUpdater?.createReminder_completion_result = .failure(testError)
    ...
    func test_updateReminder_propagatesSuccessResult() {
        // Prepare data for mocking
        let remindAt = Date()
        let reminderResponse = MessageReminder(
            id: messageId,
        ...
        // Setup mock response from message updater
        env.messageUpdater?.updateReminder_completion_result = .success(reminderResponse)

        // Setup callback verification
        let exp = expectation(description: "updateReminder callback should be called")
        var receivedResult: Result<MessageReminder, Error>?
        ...
        // Assert messageUpdater is called with correct params
        XCTAssertNotNil(env.messageUpdater)
        XCTAssertEqual(env.messageUpdater?.updateReminder_messageId, messageId)
        XCTAssertEqual(env.messageUpdater?.updateReminder_cid, cid)
        XCTAssertEqual(env.messageUpdater?.updateReminder_remindAt, remindAt)
        
        // Assert callback is called on the callback queue
        XCTAssertEqual(receivedResult?.value?.id, reminderResponse.id)
    }

    func test_updateReminder_propagatesFailureResult() {
        // Setup mock error response from message updater
        let testError = TestError()
        env.messageUpdater?.updateReminder_completion_result = .failure(testError)
    ...
    func test_deleteReminder_propagatesSuccess() {
        // Setup mock response from message updater
        env.messageUpdater?.deleteReminder_error = nil
        
        // Setup callback verification
        let exp = expectation(description: "deleteReminder callback should be called")
        var receivedError: Error?
        ...
        // Assert messageUpdater is called with correct params
        XCTAssertEqual(env.messageUpdater?.deleteReminder_messageId, messageId)
        XCTAssertEqual(env.messageUpdater?.deleteReminder_cid, cid)
        
        // Assert callback is called with nil error on success
        XCTAssertNil(receivedError)
    }

    func test_deleteReminder_propagatesError() {
        // Setup mock error response from message updater
        let testError = TestError()
        env.messageUpdater?.deleteReminder_error = testError
        ...
        // Assert callback is called with the error
        XCTAssertEqual(receivedError as? TestError, testError)
    }
}
