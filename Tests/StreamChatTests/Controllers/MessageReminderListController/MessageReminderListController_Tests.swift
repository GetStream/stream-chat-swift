...
final class MessageReminderListController_Tests: XCTestCase {
    ...
    @MainActor func test_observer_triggerDidChangeReminders_remindersHaveCorrectOrder() throws {
        ...
        try client.databaseContainer.writeSynchronously { session in
            ...
            try session.saveMessage(payload: .dummy(messageId: messageId1), syncOwnReactions: false, cache: nil)
            try session.saveMessage(payload: .dummy(messageId: messageId2), syncOwnReactions: false, cache: nil)
            try session.saveMessage(payload: .dummy(messageId: messageId3), syncOwnReactions: false, cache: nil)
