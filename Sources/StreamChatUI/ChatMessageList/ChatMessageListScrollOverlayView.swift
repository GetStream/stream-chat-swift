...
/// A protocol for `ChatMessageCollectionScrollOverlayView` data source.
public protocol ChatMessageCollectionScrollOverlayDataSource: AnyObject {
    /// Get date for item at given index path
    /// - Parameters:
    ///   - overlay: A view requesting date
    ///   - indexPath: An index path that should be used to get the date
    func scrollOverlay(_ overlay: ChatMessageCollectionScrollOverlayView, textForItemAt indexPath: IndexPath) -> String?
}

/// View that is displayed as top overlay when message list is scrolling
open class ChatMessageCollectionScrollOverlayView: _View, ThemeProvider {
    /// The displayed content.
    open var content: String? {
        didSet { updateContentIfNeeded() }
    }
    ...
    /// The data source used to get the content to display.
    public weak var dataSource: ChatMessageCollectionScrollOverlayDataSource?

    /// The collection view that is listened for being scrolled.
    public weak var collectionView: UICollectionView? {
        didSet {
            collectionView?.accessibilityIdentifier = "listView"
            contentOffsetObservation = collectionView?.observe(\.contentOffset) { [weak self] tb, _ in
                guard let self = self else { return }
                ...
                guard
                    let refPointInListView = self.superview?.convert(refPoint, to: tb),
                    let indexPath = tb.indexPathForItem(at: refPointInListView) ?? tb.indexPathsForVisibleItems.max()
                else { return }
            ...
            oldValue?.panGestureRecognizer.removeTarget(self, action: #selector(scrollStateChanged))
            collectionView?.panGestureRecognizer.addTarget(self, action: #selector(scrollStateChanged))
        }
    }
