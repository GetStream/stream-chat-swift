...
open class RecordingViewManager: AudioRecordingDelegate, AudioPlayingDelegate {
    ...
    open lazy var audioAnalysisFactory: AudioAnalysisEngine = .init(
    ...
    open func waveformDataForCurrentRecordingSession(
        ...
        if let location = location {
            audioAnalysisFactory
                .waveformVisualisation(fromAudioURL: location, for: 100) { [weak self] result in
                    switch result {
                    case let .success(waveform):
                        self?.recordingSession.waveform = waveform
