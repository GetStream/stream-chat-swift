...
extension ImageProcessors {
    ...
    struct Resize: ImageProcessing, Hashable, CustomStringConvertible {
        ...
        typealias ContentMode = ImageProcessingOptions.ContentMode
        ...
        init(size: CGSize, unit: ImageProcessingOptions.Unit = .points, contentMode: ImageProcessingOptions.ContentMode = .aspectFill, crop: Bool = false, upscale: Bool = false) {
...
struct ImageTargetSize: Hashable {
    ...
    init(size: CGSize, unit: ImageProcessingOptions.Unit) {
        switch unit {
        case .pixels: cgSize = size // The size is already in pixels
        case .points: cgSize = size.scaled(by: Screen.scale)
        }
    }
