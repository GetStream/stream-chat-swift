...
extension ImagePipeline.Cache {
    ...
    func cachedImage(for request: ImageRequest, caches: Caches = [.all]) -> ImageContainer? {
    ...
    ///   to retrieve an original image (if it's stored).
    ///   - caches: `[.all]`, by default.
    func storeCachedImage(_ image: ImageContainer, for request: ImageRequest, caches: Caches = [.all]) {
        if caches.contains(.memory) {
            storeCachedImageInMemoryCache(image, for: request)
        }
        if caches.contains(.disk) {
    ...
    func removeCachedImage(for request: ImageRequest, caches: Caches = [.all]) {
        if caches.contains(.memory) {
            removeCachedImageFromMemoryCache(for: request)
        }
        if caches.contains(.disk) {
    ...
    func containsCachedImage(for request: ImageRequest, caches: Caches = [.all]) -> Bool {
    ...
    func cachedData(for request: ImageRequest) -> Data? {
    ...
    /// - note: Default ``DataCache`` stores data asynchronously, so it's safe
    /// to call this method even from the main thread.
    func storeCachedData(_ data: Data, for request: ImageRequest) {
    ...
    func containsData(for request: ImageRequest) -> Bool {
        guard let dataCache = dataCache(for: request) else {
            return false
        }
    ...
    func removeCachedData(for request: ImageRequest) {
        guard let dataCache = dataCache(for: request) else {
            return
        }
        let key = makeDataCacheKey(for: request)
