...
final class ImagePipeline: @unchecked Sendable {
    ...
    private static let _shared = NukeAtomic(value: ImagePipeline(configuration: .withURLCache))
    ...
    init(configuration: Configuration = Configuration(), delegate: (any ImagePipelineDelegate)? = nil) {
        ...
        tasksLoadData = TaskPool(isCoalescingEnabled)
        tasksLoadImage = TaskPool(isCoalescingEnabled)
        tasksFetchOriginalImage = TaskPool(isCoalescingEnabled)
        tasksFetchOriginalData = TaskPool(isCoalescingEnabled)
    ...
    private func dispatchCallback(to callbackQueue: DispatchQueue?, _ closure: @escaping () -> Void) {
        ...
        if callbackQueue === queue {
            ...
            (callbackQueue ?? configuration._callbackQueue).async {
                box.value()
            }
        }
    }
