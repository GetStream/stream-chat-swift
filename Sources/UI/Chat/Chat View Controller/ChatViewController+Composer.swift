...
extension ChatViewController {
    ...
    func setupComposerView() {
        ...
        composerView.textView.rx.text
            ...
            .filter { [weak self] in !$0.isBlank && (self?.presenter?.channel.config.typingEventsEnabled ?? false) }
            .flatMapLatest { [weak self] _ in self?.presenter?.rx.sendEvent(isTyping: true) ?? .empty() }
            .debounce(.seconds(3), scheduler: MainScheduler.instance)
            .flatMapLatest { [weak self] _ in self?.presenter?.rx.sendEvent(isTyping: false) ?? .empty() }
            .subscribe()
            .disposed(by: disposeBag)
