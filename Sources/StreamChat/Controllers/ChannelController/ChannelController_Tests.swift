...
class ChannelController_Tests: XCTestCase {
    ...
    func test_threadReplies_areNotShownInChannel() throws {
        ...
        // Insert two messages
        let message1: MessagePayload = .dummy(messageId: "msg1-" + .unique)
        let message2: MessagePayload = .dummy(messageId: "msg2-" + .unique)
...
//                arguments: .unique,
    ...
    func test_loadPinnedMessages_propagatesMessagesFromUpdater() throws {
        // Simulate `loadPinnedMessages` call and catch the completion
        var completionMessages: [ChatMessage]?
        controller.loadPinnedMessages { [callbackQueueID] in
            AssertTestQueue(withId: callbackQueueID)
            completionMessages = try? $0.get()
        }
        
        // Simulate API response
        let payload = PinnedMessagesPayload(messages: [
            .dummy(messageId: .unique),
            .dummy(messageId: .unique),
            .dummy(messageId: .unique)
        ])
        env.channelUpdater!.loadPinnedMessages_completion!(.success(payload))
        
        // Messages are converted to models and propagated to completion
        AssertAsync.willBeEqual(
            completionMessages?.map(\.id),
