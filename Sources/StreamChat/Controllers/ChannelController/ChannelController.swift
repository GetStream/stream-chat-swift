...
public class ChatChannelController: DataController, DelegateCallable, DataStoreProvider, @unchecked Sendable {
    ...
    public func deleteFile(url: String, completion: (@MainActor @Sendable(Error?) -> Void)? = nil) {
        ...
        updater.deleteFile(in: cid, url: url) { error in
            self.callback {
                completion?(error)
            }
        }
    ...
    public func deleteImage(url: String, completion: (@MainActor @Sendable(Error?) -> Void)? = nil) {
        ...
        updater.deleteImage(in: cid, url: url) { error in
            self.callback {
                completion?(error)
            }
        }
...
private extension ChatChannelController {
    func synchronize(
        ...
        updater.update(
            ...
            onChannelCreated: { cid in self.callback { channelCreatedCallback?(cid) } },
            completion: { result in
                switch result {
                case .success:
    ...
    private func channelFeatureDisabled(
        feature: String,
    ...
    private func channelCreated(forwardErrorTo completion: (@MainActor @Sendable(_ error: Error?) -> Void)?) -> (@MainActor @Sendable(ChannelId) -> Void) {
