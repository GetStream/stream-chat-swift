...
/// Observes changes of a single entity specified using an `NSFetchRequest`in the provided `NSManagedObjectContext`.
class TestOnly_EntityDatabaseObserver<Item, DTO: NSManagedObject> {
    ...
    init(
        ...
        changeAggregator.onDidChange = { [weak self] _ in
            self?.processItem { _ in }
        }
    }
    ...
    private func processItem(completion: @escaping (Item?) -> Void) {
        ...
        log.assert(
            objects.count <= 1,
            "EntityDatabaseObserver predicate must match exactly 0 or 1 entities. Matched: \(objects)"
        )

        guard let dto = objects.first else {
            completion(nil)
            return
        }

        context.perform { [weak self] in
            guard let item = try? itemCreator(dto) else {
                completion(nil)
                return
            }
...
extension TestOnly_EntityDatabaseObserver {
    /// A builder-function that adds new listener to the current instance and returns it
    @discardableResult
    func onChange(do listener: @escaping (EntityChange<Item>) -> Void) -> TestOnly_EntityDatabaseObserver {
        listeners.append(listener)
        return self
    }
    ...
    func onFieldChange<Value: Equatable>(
        _ keyPath: KeyPath<Item, Value>,
        do listener: @escaping (EntityChange<Value>) -> Void
    ) -> TestOnly_EntityDatabaseObserver {
        // The value that stores the last received `EntityChange<Value>` and is captured by ref by the closure
        var lastChange: EntityChange<Value>?
