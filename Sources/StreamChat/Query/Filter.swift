...
extension Comparable {
    func isLess(_ other: any Comparable) -> Bool {
        guard let other = other as? Self else {
            return false
        }
        return self < other
...
public struct AnyFilterValue: Equatable, Comparable, Hashable {
    let value: FilterValue
    let valueArray: [AnyFilterValue]?
    let valueSet: Set<AnyFilterValue>?

    init(
        _ value: FilterValue
    ) {
        self.value = value
        let valueArray = (value as? [FilterValue])?.map { $0.eraseToAnyFilterValue() }
        self.valueArray = valueArray
        valueSet = valueArray.map { Set($0) }
    }

    public static func == (lhs: AnyFilterValue, rhs: AnyFilterValue) -> Bool {
        guard
            let equatableLhs = lhs.value as? any Equatable,
            let equatableRhs = rhs.value as? any Equatable
        else { return false }
    ...
    public static func < (lhs: AnyFilterValue, rhs: AnyFilterValue) -> Bool {
        guard
            let comparableLhs = lhs.value as? any Comparable,
            let comparableRhs = rhs.value as? any Comparable
        else { return false }
        return comparableLhs.isLess(comparableRhs)
    }
    ...
    public func hash(
        ...
        if let hashableValue = value as? any Hashable {
            ...
            hasher.combine(UUID()) // Combine an always random value to avoid collisions
        }
    }

    public func contains(_ filterValue: AnyFilterValue) -> Bool {
        if let arrayValue = value as? [FilterValue] {
            return arrayValue.first { $0.eraseToAnyFilterValue() == filterValue } != nil
        } else if let stringValue = value as? String, let lookingFor = filterValue.value as? String {
            return stringValue.contains(lookingFor)
        } else {
            return false
        }
    }
