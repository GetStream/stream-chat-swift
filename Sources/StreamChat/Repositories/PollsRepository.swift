...
class PollsRepository: @unchecked Sendable {
    ...
    func createPoll(
        ...
        completion: @escaping @Sendable (Result<PollResponseData, Error>) -> Void
    ) {
        ...
        ) { (result: Result<PollResponse, Error>) in
            switch result {
    ...
    func deletePoll(
        ...
        apiClient.request(
            endpoint: Endpoint<EmptyResponse>.deletePoll(pollId: pollId, userId: nil)
        ) { [weak self] in
            if $0.error == nil {
                self?.database.write { session in
    ...
    func queryPollVotes(
        ...
        ) { [weak self] (result: Result<PollVotesResponse, Error>) in
            switch result {
            case let .success(payload):
                nonisolated(unsafe) var votes: [PollVote] = []
                self?.database.write({ session in
                    votes = try session.savePollVotes(response: payload, query: query, cache: nil).map { try $0.asModel() }
                }, completion: { error in
                    if let error = error {
                        completion?(.failure(error))
                    } else {
                ...
                case let .success(response):
                    self.database.write { session in
                        for payload in response.votes {
                            try session.savePollVote(response: payload, query: nil, cache: nil)
                        }
                    } completion: { _ in
                        completion?(result)
                    }
