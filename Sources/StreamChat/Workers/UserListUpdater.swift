...
class UserListUpdater: Worker, @unchecked Sendable {
    ...
    func fetch(
        ...
        let filterConditions = userListQuery.filter?.toRawJSONDictionary() ?? [:]
        let payload = QueryUsersPayload(
            ...
            sort: userListQuery.sort.map { SortParamRequestOpenAPI(direction: $0.isAscending ? 1 : -1, field: $0.key.rawValue) }
        )
        apiClient.request(
...
extension UserListUpdater {
    ...
    func fetch(userListQuery: UserListQuery, pagination: Pagination) async throws -> [ChatUser] {
        ...
        return payload.userPayloads.map { $0.asModel() }
    }
