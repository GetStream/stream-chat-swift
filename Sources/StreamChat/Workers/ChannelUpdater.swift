...
class ChannelUpdater: Worker, @unchecked Sendable {
    ...
    func update(
        ...
        completion: (@Sendable (Result<ChannelStateResponse, Error>) -> Void)? = nil
    ) {
        ...
        let completion: @Sendable (Result<ChannelStateResponse, Error>) -> Void = { [weak database] result in
            do {
                let pagination = channelQuery.pagination
                ...
                database?.write { session in
                    ...
                    let memberListQuery = ChannelMemberListQuery(cid: cid, sort: actions?.updateMemberList ?? [])
    ...
    ///   - completion: Called when the API call is finished. Called with `Error` if the remote update fails.
    func markRead(cid: ChannelId, userId: UserId, completion: (@Sendable (Error?) -> Void)? = nil) {
    ...
    ///   - completion: Called when the API call is finished. Called with `Error` if the remote update fails.
    func channelWatchers(query: ChannelWatcherListQuery, completion: (@Sendable (Result<ChannelStateResponse, Error>) -> Void)? = nil) {
    ...
    func loadPinnedMessages(
        ...
        apiClient.request(
            ...
            case let .success(payload):
                ...
                self?.database.write { (session) in
                    pinnedMessages = session.saveMessages(messages: payload.messages, for: cid, syncOwnReactions: false)
...
extension ChannelUpdater {
    ...
    ) async throws -> ChannelStateResponse {
