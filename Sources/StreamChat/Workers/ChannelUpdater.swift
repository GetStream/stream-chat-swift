...
class ChannelUpdater: Worker, @unchecked Sendable {
        ...
        apiClient.request(endpoint: .updateChannel(query: channelQuery)) { [database] result in
            ...
            case .success(let payload):
                database.write(converting: { session in
                    ...
                    return payload.members.compactMapLoggingError { try session.member(userId: $0.userId, cid: cid)?.asModel() }
    ...
    ///   - completion: Called when the API call is finished. Called with `Error` if the remote update fails.
    func muteChannel(cid: ChannelId, mute: Bool, expiration: Int? = nil, completion: (@Sendable(Error?) -> Void)? = nil) {
        ...
        apiClient.request(
            ...
            case let .success(payload):
                self?.database.write(converting: { session in
                    session.saveMessages(messagesPayload: payload, for: cid, syncOwnReactions: false)
                        .compactMap { try? $0.asModel() }
                }, completion: { result in
                    let pinnedMessages = result.value ?? []
                    completion(.success(pinnedMessages))
                })
            case let .failure(error):
                completion(.failure(error))
            }
