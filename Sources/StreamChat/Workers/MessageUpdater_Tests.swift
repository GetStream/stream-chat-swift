...
final class MessageUpdater_Tests: XCTestCase {
    ...
    // MARK: Fetch message search query
    
    func test_fetch_makesCorrectAPICall() {
        let query = MessageSearchQuery(
            channelFilter: .exists(.cid),
            messageFilter: .equal(.text, to: .unique)
        )
        
        // Simulate `fetch` call
        messageUpdater.fetch(query: query, completion: { _ in })
        
        // Assert correct endpoint is called
        XCTAssertEqual(apiClient.request_endpoint, AnyEndpoint(.search(query: query)))
    }
    ...
    func test_muteUser_propagatesSuccessfulResponse() {
        ...
        let payload = MessageSearchResultsPayload(
            results: [
                .init(message: .dummy(messageId: .unique)),
                .init(message: .dummy(messageId: .unique)),
                .init(message: .dummy(messageId: .unique)),
                .init(message: .dummy(messageId: .unique))
            ]
        ...
        apiClient.test_simulateResponse(
            Result<MessageSearchResultsPayload, Error>.success(payload)
        )

        // Assert completion is called with messages
        AssertAsync.willBeEqual(
            messages.map(\.id),
            payload.results.map(\.message.id)
        )
    }
    ...
    func test_muteUser_propagatesError() {
        ...
        var completionCalledError: Error?
        messageUpdater.fetch(query: query) { result in
            completionCalledError = result.error
        }
        
        // Simulate API response with failure
        let error = TestError()
        apiClient.test_simulateResponse(
            Result<MessageSearchResultsPayload, Error>.failure(error)
        )
    ...
    func test_fetch_doesNotRetainUpdater() {
        let query = MessageSearchQuery(
            channelFilter: .exists(.cid),
            messageFilter: .equal(.text, to: .unique)
        )
        ...
        // Simulate `fetch` call
        messageUpdater.fetch(query: query) { _ in }
        
        // Assert upadter can be released
        AssertAsync.canBeReleased(&messageUpdater)
    }
}
