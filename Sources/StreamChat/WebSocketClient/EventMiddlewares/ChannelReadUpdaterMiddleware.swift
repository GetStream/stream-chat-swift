...
struct ChannelReadUpdaterMiddleware: EventMiddleware {
    ...
    private func adjustUnreadChannelCountsForGroupChange(
        ...
        guard let cid = try? ChannelId(cid: event.channel.cid),
              let channelDTO = session.channel(cid: cid) else { return }
        ...
        let newGroup = event.channel.custom[GroupedChannelKey.group]?.stringValue?
