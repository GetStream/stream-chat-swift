...
public class LivestreamChat: AppStateObserverDelegate, @unchecked Sendable {
    ...
    public func deleteReaction(from messageId: MessageId, with type: MessageReactionType) async throws {
        try await apiClient.request(
            endpoint: Endpoint<DeleteReactionResponse>.deleteReaction(
                id: messageId,
                type: type.rawValue,
                userId: nil
            )
    ...
    public func loadReactions(
        ...
        let payload: QueryReactionsResponse = try await apiClient.request(
            endpoint: Endpoint<QueryReactionsResponse>.queryReactions(
                id: messageId,
                queryReactionsRequest: QueryReactionsRequest(limit: limit)
            )
        )
