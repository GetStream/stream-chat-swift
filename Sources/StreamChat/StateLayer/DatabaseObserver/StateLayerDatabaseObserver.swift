...
extension StateLayerDatabaseObserver where ResultType == EntityResult {
    ...
    func startObserving(didChange: @escaping @MainActor(Item?) async -> Void) throws -> Item? where Item: Sendable {
...
extension StateLayerDatabaseObserver where ResultType == ListResult {
    ...
    func startObserving(didChange: @escaping @MainActor(StreamCollection<Item>) async -> Void) throws -> StreamCollection<Item> where Item: Sendable {
